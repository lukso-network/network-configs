...
# TODO: add --fork-choice $FORK_CHOICE when flag will be ready
...
yq eval '.CONFIG_NAME = "'"$NETWORK-$NAMESPACE"'"' $dst -i
yq eval '.MIN_GENESIS_TIME = '$GENESIS_TIME $dst -i
yq eval '.DEPOSIT_CHAIN_ID = '$CHAIN_ID $dst -i
yq eval '.DEPOSIT_NETWORK_ID = '$NETWORK_ID $dst -i

# TODO: add --fork-choice $FORK_CHOICE when flag will be ready
./genesis-state-gen --deposit-json-file ./networks/$NETWORK/$NAMESPACE/deposit_data.json \
